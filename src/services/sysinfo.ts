...
export function getSysInfo() {
    ...
    const diskUsageRaw = execSync(`df -hT | awk '{print $2 "${delimeter}" $3 "${delimeter}" $4 "${delimeter}" $5}'`).toString();
    ...
    const diskUsage = diskUsageRaw.split('\n').slice(1, -1).map(entry => {
        const [type, size, used, available] = entry.split(delimeter);
        return { type, size, used, available };
    });
